    ...
    <div id="links">
        <ul>
            ...
            <li v-if="isUserLoggedIn == true">
                <span @click="showUserWallet()" id="highlight"> {{ shortWalletAddress }}</span>
                <span @click="userLogout()"> Logout </span>
            </li>
            <li v-else>
                <span @click="connectWallet()" id="highlight"> Connect Wallet </span>
            </li>
        </ul>
    </div>
    </div>
...
export default {
    ...
    computed: {
        shortWalletAddress() {
            return this.walletAddress.substr(0, 5) + "..." + this.walletAddress.substr(-3);
        }
    },
    components: {},
    created() {
